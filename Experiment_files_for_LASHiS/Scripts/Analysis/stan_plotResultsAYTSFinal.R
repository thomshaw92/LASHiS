...
baseDirectory <- '/home/uqayork/Desktop/LASHiS/reproducibleTest/' #'C:/Users/uqayork/Documents/CrossLong-master/'
...
dktRegions <- read.csv( paste0( dataDirectory, 'dktAYTS.csv' ) )
...
stanAllResultsFile <- paste0( dataDirectory, 'stan_ResultsAllAYTS.csv' )
...
if( file.exists( stanAllResultsFile ) )
  ...
  isMultipleTimePointSubject <- rep( 1, nrow( corticalThicknessData[[1]] ) )
    ...
    for( i in 1:length( corticalThicknessData ) )
      ...
      if( nrow( corticalThicknessDataSubject ) > 1 )
        ...
        for( k in 2:nrow( corticalThicknessDataSubject ) )
          {
          span <- interval( dmy( corticalThicknessDataSubject$EXAM_DATE[1] ), dmy( corticalThicknessDataSubject$EXAM_DATE[k] ) )
  ...
  for( i in 1:length( corticalThicknessCsvs ) )
    {
    stanResultsFiles[i] <- paste0( dataDirectory, 'stan_', corticalThicknessPipelineNames[i], '_ResultsAYTS.csv' )    
    }  

  stanModelFile <- paste0( dataDirectory, 'stan_corticalThicknessModelAYTS.stan' )
...
write.csv( stanResultsAll, quote = FALSE, row.names = FALSE, 
           file = paste0( dataDirectory, "stan_ResultsAllAYTS.csv" ) )  
...
ggsave( paste0( figuresDirectory, "sigma_FINALX_AYTS.pdf" ), sigmaPlot, width = 10, height = 3 )
...
tauPlot <- ggplot( data = stanResultsAll, aes( y = tau0.50., x = DktRegion, colour = Pipeline, shape = Pipeline ) ) +
              ...
              theme( legend.position = "right" )
ggsave( paste0( figuresDirectory, "tau_FINALX_AYTS.pdf" ), tauPlot, width = 10, height = 3 )
...
variance.ratioPlot <- ggplot ( data = stanResultsAll, aes( y = variance.ratio.50., x = DktRegion, colour = Pipeline, shape = Pipeline) ) +
  ...
  theme( legend.position = "right" )
ggsave( paste0( figuresDirectory, "variance.ratio_FINALX_AYTS.pdf" ), variance.ratioPlot, width = 10, height = 3 )
...
boxPlot <- ggplot( data = allDataResults, aes( x = Pipeline, y = X50., fill = Pipeline ) ) +
              ...
              labs( x = '', y = '' )
ggsave( paste0( figuresDirectory, "allData_FINALX_AYTS.pdf" ), boxPlot, width = 10, height = 4 )
...
ggsave( paste0( figuresDirectory, "allData_boxResidualVar_AYTS.pdf" ), boxPlotResidualVar, width = 10, height = 4 )
...
ggsave( paste0( figuresDirectory, "allData_boxBetweenVar_AYTS.pdf" ), boxPlotBetweenVar, width = 10, height = 4 )
...
ggsave( paste0( figuresDirectory, "allData_boxVarRatio_AYTS.pdf" ), boxPlotVarRatio, width = 10, height = 4 )
