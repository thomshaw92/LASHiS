...
#baseDirectory <- '/home/uqayork/Desktop/CrossLong-master/'#'Q:/uqtshaw/transfer_folder/LASHiS_figures_results_etc/LASHiS_experiment_one/' 
baseDirectory <- 'Q:/uqtshaw/transfer_folder/LASHiS_figures_results_etc/LASHiS_experiment_one/'
dataDirectory <- paste0( baseDirectory, 'DataDummy/')
sandboxDirectory <- paste0( baseDirectory, 'Sandbox/')
figuresDirectory <- paste0( baseDirectory, 'FiguresDummy/')
...
dktRegion <- bayesMeans$DktRegion
...
#ayesMeansPlotVol <- ggplot (data = bayesMeans, aes( fill=factor(Pipeline, levels=positions), y = mean_volSim, x = dktRegion )) +
###BOXPLOTS
## volSim mean boxplot  
bayesMeansBoxPlotVol <- ggplot (data = bayesMeans, aes(fill = factor(Pipeline, levels=c( "Freesurfer Xs", "ASHS Xs", "Freesurfer Long", "Diet LASHiS", "LASHiS")), x = Pipeline, y = mean_volSim ) ) +
  ...
  geom_point (aes (shape=dktRegion), na.rm=TRUE, position=position_dodge2(width = .5), size = 4)+
...
###ASH THIS IS MY ATTEMPT BUT IT FAILED, oh, but I fixed it <3
...
newDktRegion = factor( c( rep( dktBrainGraphRegions, 10 ) ))
...
## volSim mean plot  
bayesMeansPlotVol <- ggplot (data = bayesMeans, aes( fill=factor(Pipeline, levels=positions), y = mean_volSim, x = dktRegion )) +
  ...
  # dioce mean plot  
  bayesMeansPlotdice <- ggplot (data = bayesMeans, aes( fill=factor(Pipeline, levels=positions), y = mean_dice, x = dktRegion )) +
    scale_fill_manual(values = c( fsXsCol, ashsXsCol, fsLongCol, ashsDiet , ashsLongCol ) ) +
    scale_colour_manual(values = c( fsXsCol, ashsXsCol, fsLongCol, ashsDiet , ashsLongCol ) ) +
    ylim(c(0, 1.2))+
    geom_bar(position="dodge", stat="identity")+
