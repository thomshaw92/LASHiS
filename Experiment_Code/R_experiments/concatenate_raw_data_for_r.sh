...
#Long ##FIXME to be recompiled
cp ${base_dir}/Freesurfer/${subjName}_ses $base_dir/Freesurfer/${subjName}_Freesurfer/$var/mprage1${side}SSTLabelsWarpedToTimePoint0_stats.txt
cp ${base_dir}/Freesurfer/${subjName}_ses $base_dir/Freesurfer/${subjName}_Freesurfer/$var/mprage2${side}SSTLabelsWarpedToTimePoint1_stats.txt
cp ${base_dir}/Freesurfer/${subjName}_ses $base_dir/Freesurfer/${subjName}_Freesurfer/$var/mprage3${side}SSTLabelsWarpedToTimePoint2_stats.txt
...
#sub = rows 2 4 6 8
    if [[ -e ${left_tp1_vols} && ${right_tp1_vols} ]] ; then 
	LCA11="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e1)/(e1+e2+e3+e4+e6)}' $left_tp1_vols)" 
	LCA231="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e2+e4)/(e1+e2+e3+e4+e6)}' $left_tp1_vols)"
	LDG1="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e3)/(e1+e2+e3+e4+e6)}' $left_tp1_vols)"
	LSUB1="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e6)/(e1+e2+e3+e4+e6)}' $left_tp1_vols)" 
	RCA11="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e1)/(e1+e2+e3+e4+e6)}' $right_tp1_vols)"  
	RCA231="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e2+e4)/(e1+e2+e3+e4+e6)}' $right_tp1_vols)"
	RDG1="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e3)/(e1+e2+e3+e4+e6)}' $right_tp1_vols)"
	RSUB1="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e6)/(e1+e2+e3+e4+e6)}' $right_tp1_vols)" 
    fi	
    if [[ -e ${left_tp2_vols} && ${right_tp2_vols} ]] ; then 
	LCA12="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e1)/(e1+e2+e3+e4+e6)}' $left_tp2_vols)"
	LCA232="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e2+e4)/(e1+e2+e3+e4+e6)}' $left_tp2_vols)"
	LDG2="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e3)/(e1+e2+e3+e4+e6)}' $left_tp2_vols)"
	LSUB2="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e6)/(e1+e2+e3+e4+e6)}' $left_tp2_vols)"
	RCA12="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e1)/(e1+e2+e3+e4+e6)}' $right_tp2_vols)"
	RCA232="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e2+e4)/(e1+e2+e3+e4+e6)}' $right_tp2_vols)"
	RDG2="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e3)/(e1+e2+e3+e4+e6)}' $right_tp2_vols)"
	RSUB2="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e6)/(e1+e2+e3+e4+e6)}' $right_tp2_vols)"
    fi
    if [[ -e ${left_tp3_vols} && ${right_tp3_vols} ]] ; then 
	LCA13="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e1)/(e1+e2+e3+e4+e6)}' $left_tp3_vols)"
	LCA233="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e2+e4)/(e1+e2+e3+e4+e6)}' $left_tp3_vols)"
	LDG3="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e3)/(e1+e2+e3+e4+e6)}' $left_tp3_vols)"
	LSUB3="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e6)/(e1+e2+e3+e4+e6)}' $left_tp3_vols)"
	RCA13="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e1)/(e1+e2+e3+e4+e6)}' $right_tp3_vols)"
	RCA233="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e2+e4)/(e1+e2+e3+e4+e6)}' $right_tp3_vols)"
	RDG3="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e3)/(e1+e2+e3+e4+e6)}' $right_tp3_vols)"
	RSUB3="$(awk -v x=2 -v a=1 -v b=2 -v c=4 -v d=3 -v s=6 'NR==a{e1=$x} NR==b{e2=$x} NR==c{e3=$x} NR==d{e4=$x} NR==s{e6=$x} END{if (x) print (e6)/(e1+e2+e3+e4+e6)}' $right_tp3_vols)" 
    fi
    ...
    then 
	echo "$subjName one or more varibales are empty" >> $base_dir/ADNI_DATA_missing_files.txt
	echo "skipping participant ${subjName}"
    else 
	echo "Data is all good" 
	
	awk -v lca11=$LCA11 -v lca231=$LCA231 -v ldg1=$LDG1 -v lsub1=$LSUB1 -v rca11=$RCA11 -v rca231=$RCA231 -v rdg1=$RDG1 -v rsub1=$RSUB1 'BEGIN{FS=OFS=","} NR==1 {print $0","lca11","lca231","ldg1","lsub1","rca11","rca231","rdg1","rsub1}1' $base_dir/Freesurfer/${subjName}_Freesurfer/${subjName}_demo_info.csv > $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv && cat $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv > $base_dir/Freesurfer/${subjName}_Freesurfer/${subjName}_${var}_final.csv && rm $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv 

	awk -v lca12=$LCA12 -v lca232=$LCA232 -v ldg2=$LDG2 -v lsub2=$LSUB2 -v rca12=$RCA12 -v rca232=$RCA232 -v rdg2=$RDG2 -v rsub2=$RSUB2 'BEGIN{FS=OFS=","} NR==2 {print $0","lca12","lca232","ldg2","lsub2","rca12","rca232","rdg2","rsub2}1' $base_dir/Freesurfer/${subjName}_Freesurfer/${subjName}_demo_info.csv > $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv && cat $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv > $base_dir/Freesurfer/${subjName}_Freesurfer/${subjName}_${var}_final.csv && rm $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv 

	if [[ -z "$LCA13" || -z "$LCA233" || -z "$LDG3" || -z "$LSUB3" || -z "$RCA13" || -z "$RCA233" || -z "$RDG3" || -z "$RSUB3" || -z "$demo" ]] 
	then
	    echo "no timepoint 3 for $subjName" >>  $base_dir/Freesurfer/ADNI_DATA_missing_files.txt
	else
	    awk -v lca13=$LCA13 -v lca233=$LCA233 -v ldg3=$LDG3 -v lsub3=$LSUB3 -v rca13=$RCA13 -v rca233=$RCA233 -v rdg3=$RDG3 -v rsub3=$RSUB3 'BEGIN{FS=OFS=","} NR==3 {print $0","lca13","lca233","ldg3","lsub3","rca13","rca233","rdg3","rsub3}1' $base_dir/Freesurfer/${subjName}_Freesurfer/${subjName}_demo_info.csv > $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv && cat $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv > $base_dir/Freesurfer/${subjName}_Freesurfer/${subjName}_${var}_final.csv && rm $base_dir/Freesurfer/${subjName}_Freesurfer/temp.csv 	
	fi
	#then you just have to concatenate all the subjnames and for each var.
